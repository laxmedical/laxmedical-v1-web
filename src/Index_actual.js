...
const Home = () => {
  ...
  const makeCall = () => {
    ...
    const peerToConnect = connectedPeers.map((peer) => peer);
    const myPeerIndex = peerToConnect.findIndex((peer) => peer === myPeerID);
            ...
            <div className="listeners-box">
              {remoteStreams.map((dataStream, i) => (
                <div key={dataStream.peerId} className="listener-stream">
                  <video ref={(ref) => (refsArray.current[dataStream.peerId] = ref)} autoPlay playsInline muted />
                </div>
              ))}
            </div>
          </div>
