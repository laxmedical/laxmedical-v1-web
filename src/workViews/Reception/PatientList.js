      ...
      run: (data) => {
        ...
        if (data.type === 'patient_list' && data.status !== 'error') {
          const d = data.content.patients.map((p) => {
            const patientAge = new Date(new Date() - new Date(p.birth_date));
            const age = patientAge.getYear() - 70;
            ...
            <TableBody>
              {stableSort(rows, getComparator(order, orderBy))
                ...
                .map((row, index) => {
                  ...
                  return (
                    <TableRow hover checked={isItemSelected} tabIndex={-1} key={row.name} selected={isItemSelected}>
                      <TableCell>
                      ...
                      <TableCell>
                        {row.protein} | <Link to={`/lax_medic/work-place/patient-profiles-${row.id}/enregistrer-une-fiche`}>Ajouter</Link>
                      </TableCell>
                    </TableRow>
                  );
                })}
